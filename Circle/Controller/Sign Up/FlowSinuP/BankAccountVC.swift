...
class BankAccountVC: UIViewController, STPPaymentCardTextFieldDelegate {
    ...
    func setupView() {
        ...
        headline.text = "Add your debit card"
   ...
   @objc func nextStep() {
    ...
    STPAPIClient.shared().createToken(withCard: cardField.cardParams) { (token, error) in
        if let error = error {
            ...
            DataService.instance.saveBankInformation(email: email, password: self.password, tokenId: token!.tokenId) { (success, error) in
                if !success {
                    dispatch.async {
                        self.nextButton.hideLoading()
                        alert.showPromptMessage(self, title: "Error", message: (error?.localizedDescription)!)
