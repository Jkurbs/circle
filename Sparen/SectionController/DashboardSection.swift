...
class DashboardSection: ListSectionController {
    ...
    override init() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(self.updateCircleView(_:)), name: NSNotification.Name(rawValue: "userActivities"), object: nil)
    }
    ...
    @objc func updateCircleView(_ notification: NSNotification) {
        if let dict = notification.userInfo as NSDictionary? {
            if let daysLeft = dict["days_left"] as? Int, let daysTotal = dict["days_total"] as? Int {
                print("DAYS LEFT:", daysLeft)
                if let cell = collectionContext?.cellForItem(at: 0, sectionController: self) as? SelectedUserCell {
                    cell.circleView.endPointValue = CGFloat(daysLeft)
                    cell.circleView.maximumValue = CGFloat(daysTotal)
                }
            }
        }
    }
